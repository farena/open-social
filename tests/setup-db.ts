// Global vitest setup. Forces a unique TEST_DB_PATH per worker process so
...
// TEST_DB_PATH in their own beforeEach for finer-grained isolation —
...
// TEST_DB_PATH in beforeEach run after this and override it. Test files
// that don't manage the env var stay safely on the fallback.
beforeEach(() => {
  if (!process.env.TEST_DB_PATH) {
    process.env.TEST_DB_PATH = fallbackDbPath;
  }
});

afterAll(() => {
  for (const ext of ["", "-wal", "-shm"]) {
    const f = fallbackDbPath + ext;
    if (fs.existsSync(f)) {
      try {
        fs.unlinkSync(f);
      } catch {
